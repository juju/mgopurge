...
func PurgeMissing(tc, sc *mgo.Collection, collections ...string) error {
		...
		for iter.Next(&tdoc) {
			for _, txnToken := range tdoc.TxnQueue {
				if txnId, ok := tokenToId(txnToken); ok {
					if !txnExists(txnId) {
						fmt.Printf("WARNING: purging from document %s/%v the missing transaction id %s\n",
					...
					fmt.Printf("WARNING: purging from document %s/%v the invalid transaction token %#v\n",
	...
	for iter.Next(&stdoc) {
		for _, txnToken := range stdoc.TxnQueue {
			if txnId, ok := tokenToId(txnToken); ok {
				if !txnExists(txnId) {
					fmt.Printf("WARNING: purging from stash document %s/%v the missing transaction id %s\n",
				...
				fmt.Printf("WARNING: purging from stash document %s/%v the invalid transaction token %#v\n",
